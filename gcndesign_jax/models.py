...
class RGCBlock(nn.Module):
    ...
    def __call__(self, x, edgevec, adjmat, training: bool):
        jax.debug.print("RGCBloack")
        #jax.debug.print(edgevec.shape)
        L, d_in = x.shape
...
class Embedding_module(nn.Module):
    ...
    def __call__(self, node_in, edge_flat, adjmat_in, train: bool):
        jax.debug.print("Embedding_Module")
        L = node_in.shape[0]
...
class Prediction_module(nn.Module):
    ...
    def __call__(self, node_in, train: bool):
        jax.debug.print("Prediction_module")
        kernel_size = (self.fragment_size - 1) // 2 + 1
        node = node_in[None, ...]
